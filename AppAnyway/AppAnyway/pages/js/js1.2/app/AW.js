...
(function(){
	var AWModal = function(){
		...
		if(AWUserConfig && AWUserConfig.tipLangues)				
			this.tipLangues = AWErrLang[AWUserConfig.tipLangues] || AWErrLang.zh;	//确定提示语言
		else
...
(function(){
	...
	AWAnimation.prototype.init = function(){
		...
		document.getElementById('awpage0').addEventListener('webkitAnimationEnd',function(){
			me.movingFlag = false;
			me.setCurrent();
			//remove hide page html
			me.removeHtml();
		});
	}
	...
	AWAnimation.prototype.removeHtml = function(){
		...
		var last = me.current;
		if(last){
			var lastPage = AW.getDomById('page'+last);
			var children = lastPage.childNode;
			if(lastPage.childNode.length)
...
(function(){
	var AWPageAction = function(){
		var me = this;
		me.direct = AWUserConfig.transitionMode;   	//页面滑动模式
		me.pages = {};								//页面方法及参数对象
		me.loading = false;							//标识页面是否正在加载
		me.pageTransitions = new AW.AWAnimation();	//页面滑动
		me.lastPConfig = null;						//上一个页面传入的参数
		me.lastPage = "";							//上一个页面Id
	}
